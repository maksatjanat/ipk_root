...
*
* @param int $Time Временная метка в формате Unix.
* @param bool $Full Выводить время.
* @param bool $Logic Выводить в легко читаемом виде (Сегодня, Вчера, 1 Января).
* @return string
*/
function TimeRender( $Time, $Full = true, $Logic = true )
{
static $delimiter, $timezone_offset, $day, $year,
$months = array('Января', 'Февраля', 'Марта', 'Апреля', 'Мая', 'Июня',
'Июля', 'Августа', 'Сентября', 'Октября', 'Ноября', 'Декабря');
//$escape_charlist = '\\dDjlNSwzWFmMntLoYyaABgGhHisueIOPTZcrU';
if (empty($Time) || !is_numeric($Time)) return 'Нет данных';
 
 
 
 
if (!isset($delimiter)){
$delimiter = System::config('general/datetime_delimiter');
if (empty($delimiter)) $delimiter = ' ';
$timezone_offset = date('Z');
$now = time();
$day = floor(($now+$timezone_offset)/Day2Sec);
$year = date('Y', $now);
}
...
$desc = '';
if ($Logic){
$d = $day-floor(($Time+$timezone_offset)/Day2Sec);
if ($d == 0) $desc = 'Сегодня';
elseif ($d == 1) $desc = 'Вчера';
elseif ($d == -1) $desc = 'Завтра';
elseif ($d == -2) $desc = 'Послезавтра';
}
...
if (empty($desc)){
if ($Logic && date('Y', $Time) == $year) $date_format = 'j '.$months[date('n', $Time)-1]; //addcslashes($months[date('n', $Time)-1], $escape_charlist);
else $date_format = 'd.m.Y';
}else $date_format = $desc; //addcslashes($desc, $escape_charlist);
 
 
 
 
if ($Full) return date($date_format.$delimiter.'H:i', $Time);
else return date($date_format, $Time);
}
