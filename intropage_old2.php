...
if(!isset($_SESSION["fullname"])) {
	...
	//------------------------------------------------
	//сохранение в переменных данных сессии 
	//$queryCompany="SELECT NameCompany FROM tbl_company WHERE id_company='".$_SESSION['CompanyId']."'";
	
if ($result=mysqli_query($con,"SELECT NameCompany FROM tbl_company WHERE id_company='".$_SESSION['CompanyId']."'")) 
{
	$company=mysqli_fetch_array($result);
	$_SESSION['NameCompany']=$company[0];
 /* закрытие выборки */
...
//mysqli_close($con);
	...
	//		@$con = mysqli_connect(DB_SERVER,DB_USER, DB_PASS) or die(mysql_error());
		//	mysqli_query($con,"SET NAMES utf8 COLLATE utf8_unicode_ci");
			//Если заявка подана от юр лица то-------------------------------------
			if($_SESSION['b_employee']==1){
				...
				if (isset($_POST['idCourse']))
				{
					...
					if (mysqli_query($con, "INSERT INTO coursepers(CourseId,PersonaId,CountStudent,comment) VALUES(".$_POST['idCourse'].",".$_SESSION['idpersona'].",".$_POST['kolvo'].",'".$_POST['comment']."')") === TRUE AND mysqli_query($con, "UPDATE tbl_courses SET memb_int=".$bids_sum." WHERE id=".$_POST['idCourse']."")===TRUE) 
					{
				...
				//Иначе если заявка подана от физ лица-------------------------------------
				if (isset($_POST['idCourse'])){
							/* выводит существующее количество слушателей, наименование  */
					$insert_sum=mysqli_fetch_array(mysqli_query($con,"SELECT title,memb_int FROM tbl_courses WHERE id='".$_POST['idCourse']."'"));
					$bids_sum=$insert_sum[1]+1;
					$bids_title=$insert_sum[0];
					/* выводит Данные пользователя */
					$persona=mysqli_fetch_array(mysqli_query($con,"SELECT * FROM tbl_persona WHERE id_persona='".$_SESSION['idpersona']."'"));
					//$pers_fio=$persona[2]+' '+$persona[3]+' '+$persona[4];
			...
			<body>
			  <p>Подана заявка на курс повышения квалификации от физ лица:</p>
			  <table>
				<tr>
				  <th>Фамилия</th><th>Имя</th><th>Отчество</th><th>Емайл</th><th>Телефон</th>
				</tr>
				<tr>
				  <td>'.$persona[3].'</td><td>'.$persona[2].'</td><td>'.$persona[4].'</td><td>'.$persona[5].'</td><td>'.$persona[6].'</td><td>1970</td>
				</tr>
			  </table>
			  <p>На тему:'.$bids_title.'</p>
			</body>
			</html>
			';
			...
			//------------------------------------------------------------------------------------------------------------------------------------------------------------------------->
					...
					//INSERT INTO coursepers(CourseId,PersonaId,CountStudent) VALUES($_POST['idCourse'],$_SESSION['idpersona'],$_POST['kolvo'])
					//"UPDATE tbl_persona SET CourseId=".$_POST['voice'].", KolvoStudent=".$_POST['kolvo']." WHERE id_persona=".$_SESSION['idpersona'].""
					if (mysqli_query($con, "INSERT INTO coursepers(CourseId,PersonaId,CountStudent,comment) VALUES(".$_POST['idCourse'].",".$_SESSION['idpersona'].",1,'".$_POST['comment']."')") === TRUE AND
	...
	} else if (isset($_POST['new_proposal'])) {
		if(mysqli_query($con, 
			"INSERT INTO tbl_proposal(person_id,proposal) VALUES(". $_SESSION['idpersona']. ", '". $_POST['comment']."')") === TRUE){
			printf("Предложение добавлено.\n");
		}else mysqli_connect_error();
	}		
...
<div class="row">
	...
	<div class="col-lg-6 col-md-6 col-sm-6"style="text-align: right;"><a href="logout.php">Выйти</a> из аккаунта</br><h4><?php if ($_SESSION['b_employee']){echo $_SESSION['NameCompany'];}?></h4></div>
	</div>		
</div>		
...
?>
		...
		<div class="col-lg-6 col-md-6 col-sm-6" style="text-align: right;"> 
			<input type="submit" value="Подать заявку" name="bid_button" class="btn btn-success"/> 
		</div>
