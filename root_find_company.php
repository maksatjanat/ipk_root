...
if(isset($_POST['idCourse'])) {
	...
	$sql="SELECT p.FirstName, p.SurName, p.PhoneUser, k.NameCompany, c.CountStudent,c.idBid, 
	/** Начало изменении, Maksat **/
	c.comment 
	/** Конец изменении, Maksat **/
	FROM tbl_company k,coursepers c INNER JOIN tbl_persona p ON c.PersonaId = p.id_persona WHERE p.CompanyId=k.id_company AND c.CourseId='$idCourse'";
	...
	$string = '';
	// Начало изменении, Maksat
	$string .= '<tr><th>ID</th><th>Имя</th><th>Фамимлия</th><th>Телефонный номер</th><th>Наименование	 компании</th><th>Комментарий</th><th>Количество студентов</th></tr>';
	// Конец изменении, Maksat
